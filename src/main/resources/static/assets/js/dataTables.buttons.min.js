...
(function(e) {
    ...
    e.extend(v.prototype, {
        ...
        add: function(a, b) {
            ...
            this._expandButton(
                ...
                a,
                a !== q ? a.split : q,
                (a === q || a.split === q || 0 === a.split.length) && d !== q, !1,
                b
            );
            this._draw();
            return this;
        ...
        _expandButton: function(a, b, c, d, h, g, l) {
            ...
            for (var r = p.length; c < r; c++) {
                ...
                if (f)
                    if (
                        ((b = f.config !== q && f.config.split ? !0 : !1), Array.isArray(f))
                    )
                        this._expandButton(
                            a,
                            f,
                        ...
                        var m = this._buildButton(
                            f,
                            d,
                            f.split !== q || (f.config !== q && f.config.split !== q),
                            h
                        );
                        if (m) {
                            ...
                            if (m.conf.buttons || m.conf.split) {
                                ...
                                if (m.conf.split)
                                    for (var u = 0; u < m.conf.split.length; u++)
                                        ...
                                        m.conf.split[u].collectionLayout === q &&
                                        (m.conf.split[u].collectionLayout =
                                            m.conf.collectionLayout),
                                        m.conf.split[u].dropup === q &&
                                        (m.conf.split[u].dropup = m.conf.dropup),
                                        m.conf.split[u].fade === q &&
                                        (m.conf.split[u].fade = m.conf.fade));
                                else
                                    e(m.node).append(
                                        e(
        ...
        _popover: function(a, b, c) {
            ...
            if ("absolute" === d) {
                ...
                if (
                    ...
                    ((m = g.children("button.dt-btn-split-drop").offset().left),
                        (f = g.children("button.dt-btn-split-drop").outerWidth()),
                        (u = m + f));
                    f = 0;
                    if (c.hasClass(h.rightAlignClassName))
                        (f = r - p),
                        d > n + f &&
                        ((d -= n + f), (k -= p + f), (f = d > k ? f + k : f + d));
            ...
            h = function(g) {
                if (Array.isArray(g))
                    ...
                    "string" === typeof g ?
                        -1 !== g.indexOf(",") ?
                            h(g.split(",")) :
                            ((g = e.inArray(g.trim(), d)), -1 !== g && c.push(b[g].inst)) :
            ...
            var c = [],
                d = function(k, n, p) {
                    for (var r, f, m = 0, u = n.length; m < u; m++)
                        if ((r = n[m]))
                            (f = p !== q ? p + m : m + ""),
                                k.push({ node: r.node, name: r.conf.name, idx: f }),
                ...
                h = function(k, n) {
                    ...
                    if (Array.isArray(k) || k instanceof e)
                        ...
                        if (-1 !== k.indexOf(","))
                            for (r = k.split(","), f = 0, p = r.length; f < p; f++)
                                h(r[f].trim(), n);
                        else if (k.match(/^\d+(\-\d+)*$/))
                            ...
                            for (k = k.replace(":name", ""), f = 0, p = r.length; f < p; f++)
                                r[f].name === k && c.push({ inst: n, node: r[f].node });
                        else
                            e(f)
                                .filter(k)
                                .each(function() {
                        ...
                        "object" === typeof k &&
                        k.nodeName &&
                        ((r = e.inArray(k, f)), -1 !== r && c.push({ inst: n, node: f[r] }));
                },
                g = 0,
                l = a.length; g < l; g++
        )
    ...
    t.Api.register("buttons()", function(a, b) {
        ...
        var c = this.iterator(!0,
            ...
            function(d) {
                if (d._buttons)
                    return v.buttonSelector(v.instanceSelector(a, d._buttons), b);
            }, !0
        );
        c._groupSelector = a;
        return c;
    ...
    var K = function(a) {
        ...
        Q = function(a, b) {
            ...
            g = g.nodes().toArray();
            for (
                var l = b.length, k = [], n = 0, p = 0, r = 0 < l ? h.length / l : 0; p < r; p++
            ) {
